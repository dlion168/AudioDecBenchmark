...
def list_codec(ignore_list=['__init__.py', 'general.py', 'academicodec_hifi_16k_320d.py', 'funcodec_en_libritts_16k_gr1nq32ds320.py', 'funcodec_en_libritts_16k_gr8nq32ds320.py']):
    dataset_dir = os.path.dirname(os.path.abspath(__file__))
    files = os.listdir(dataset_dir)
    py_files = filter(lambda x: x.endswith('.py') and x not in ignore_list, files)
    # py_files = filter(lambda x: x.endswith('.py') and x in ['facodec.py'], files)    
